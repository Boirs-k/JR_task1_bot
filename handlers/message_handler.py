...
from classes.resource import BotPhoto
...
async def wait_for_gpt_handler(message: Message, state: FSMContext):
    ...
    await message.answer_photo(photo=photo, caption=gpt_response)
    await state.clear()
...
async def talk_handler(message: Message, state: FSMContext):
    ...
    await message.answer_photo(photo=data['photo'], caption=response, reply_markup=kb_end_talk())
